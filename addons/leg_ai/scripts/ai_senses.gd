...
func hear(_delta: float) -> void:
	...
	if best_sound_target:
		# LEG_Log.log("%s heard %s" % [name, best_sound_target.name])
...
func update_target_data(data: Variant, target_actor: Actor, delta: float) -> void:
	...
	if dir_to_target.length_squared() > vision_range * vision_range:
		# LEG_Log.log("%s is too far from %s to see" % [actor_self.name, target_actor.name])
		return
	...
	if not is_facing_target:
		# LEG_Log.log("%s is not facing %s" % [actor_self.name, target_actor.name])
		return
	...
	if not in_field_of_view:
		# LEG_Log.log("%s is not in field of view of %s" % [target_actor.name, actor_self.name])
		return
	...
	if result.is_empty():
		...
		# LEG_Log.log("%s can see %s - %s" % [actor_self.name, target_actor.name, data.visible_time])
	# else:
		# LEG_Log.log("%s cannot see %s due to obstruction" % [actor_self.name, target_actor.name])
